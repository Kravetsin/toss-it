 ...
 * OAuth callback holds, that a linked login must not repaint the primary identity.
 *
...
export async function refreshChatterName(
  ...
  if (!row) return;
  if (row.login === login && row.displayName === displayName) return;

  try {
    await db.update(users).set({ login, displayName }).where(eq(users.id, id));
    ...
    // it, the write fails as a whole. The visible name is what people actually read, so keep it
    // rather than leaving the whole profile stale over a slug.
    try {
      await db.update(users).set({ displayName }).where(eq(users.id, id));
      log.warn({ err, twitchId, login }, 'twitch-chat: login taken, refreshed display name only');
    } catch (err2) {
