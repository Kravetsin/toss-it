...
export class EventSubClient {
  ...
  /** broadcasterId -> subscription id (needed for DELETE on toggle-off). */
  ...
  private async subscribe(broadcasterId: string): Promise<void> {
    ...
    try {
      ...
      if (!res.ok) {
        this.deps.log.warn(
          { broadcasterId, status: res.status, body: await res.text() },
      ...
      if (subId) this.subs.set(broadcasterId, subId);
    } catch (err) {
  ...
  private async unsubscribe(broadcasterId: string): Promise<void> {
    const subId = this.subs.get(broadcasterId);
    this.subs.delete(broadcasterId);
    if (!subId) return;
    try {
      // 404 just means the subscription already died with an old session — fine.
      await this.helix('DELETE', `${HELIX_SUBS_URL}?id=${encodeURIComponent(subId)}`);
    } catch (err) {
      this.deps.log.warn({ err, broadcasterId }, 'twitch-chat: unsubscribe error');
    }
  }
}
