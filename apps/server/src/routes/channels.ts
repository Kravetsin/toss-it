...
async function chatBoard(
  ...
  const entries = rows.map((r, i) => {
    ...
    return {
      ...
      nickColor2: u?.equipped?.nickColor2 ?? null,
...
export function registerChannelRoutes(app: FastifyInstance): void {
  ...
  app.get<{ Params: { login: string } }>('/api/c/:login', async (req, reply) => {
    ...
    // streamer sees in Achievements matches this gate exactly.
    ...
    const response: PublicChannelInfo = {
      ...
      overlayLayout: { position: overlayPosition, size: overlaySize, margin: overlayMargin },
