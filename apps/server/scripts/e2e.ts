/**
 * E2E test against a running dev server. Requires fake auth + short cooldown:
 *   VIEWER_COOLDOWN_MS=1000 ALLOW_FAKE_AUTH=1 pnpm --filter @tmw/server dev
 * Run: pnpm --filter @tmw/server exec tsx scripts/e2e.ts
 */
...
const COOLDOWN_WAIT_MS = 1200; // slightly above VIEWER_COOLDOWN_MS=1000

// Unique logins per run keep the test idempotent.
...
assert(
  ...
  `новичок должен попасть в pending, получил ${first.body.status}`,
);
await waitFor(() => moderationNew.some((s) => s.id === first.body.id), 'moderation:new в дашборде');
...
await waitFor(
  () => plays.some((p) => p.submissionId === first.body.id),
  'media:play после одобрения',
);
...
await waitFor(
  () => plays.some((p) => p.submissionId === second.body.id),
  'автопоказ из белого списка',
);
...
await waitFor(
  () => moderationNew.some((s) => s.id === badFirst.body.id),
  'moderation:new для бана',
);

const rejectRes = await fetch(`${SERVER}/api/dashboard/submissions/${badFirst.body.id}/reject`, {
  method: 'POST',
  headers: { cookie: streamerCookie, 'content-type': 'application/json' },
...
await sleep(800);
assert(
  moderationNew.length === newCountBefore,
  'отправка забаненного не должна попадать в модерацию',
);
...
assert(
  wl.some((u) => u.login === VIEWER),
  'зритель не появился в белом списке',
);
const bl = (await (
  await fetch(`${SERVER}/api/dashboard/bans`, { headers: { cookie: streamerCookie } })
).json()) as ListedUser[];
assert(
  bl.some((u) => u.login === BAD_VIEWER),
  'плохой зритель не появился в банах',
);
...
assert(
  ownerTest.status === 201,
  `тест-отправка владельца: ожидал 201, получил ${ownerTest.status}`,
);
assert(
...
await waitFor(
  () => plays.some((p) => p.submissionId === shortVideo.body.id),
  'показ после настройки',
);
overlaySocket.emit('playback:done', shortVideo.body.id);
console.log('13. per-channel лимит длительности работает (20с → 5с)');

const audioSettings = (await (
  await dashFetch('/api/dashboard/settings', {
    method: 'PUT',
    body: JSON.stringify({ maxAudioDurationMs: 3000 }),
  })
).json()) as { maxDurationMs: number; maxAudioDurationMs: number };
assert(
  audioSettings.maxAudioDurationMs === 3000 && audioSettings.maxDurationMs === 5000,
  `аудио-лимит должен быть 3000 при видео 5000, получил ${JSON.stringify(audioSettings)}`,
);
const audio = await upload(
  streamerCookie,
  'scripts/fixtures/audio10.mp3',
  'audio/mpeg',
  'audio10.mp3',
);
assert(audio.status === 201, `аудио: ожидал 201, получил ${audio.status}`);
assert(
  audio.body.durationMs === 3000,
  `аудио 10с должно резаться по СВОЕМУ лимиту до 3000мс (не по видео-5000), получил ${audio.body.durationMs}`,
);
await waitFor(() => plays.some((p) => p.submissionId === audio.body.id), 'показ аудио');
overlaySocket.emit('playback:done', audio.body.id);
console.log('14. у аудио отдельный лимит длительности, не зависит от видео');
...
assert(
  closedViewer.status === 403,
  `стоп-кран: зритель должен получить 403, получил ${closedViewer.status}`,
);
...
await waitFor(
  () => plays.some((p) => p.submissionId === closedOwner.body.id),
  'показ при стоп-кране',
);
...
assert(
  blAfter.some((u) => u.login === VIEWER),
  'забаненный должен появиться в банах',
);
