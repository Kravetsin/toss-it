 ...
 * chat messages (2 pts) + watch-minutes (1 pt) + aired submissions (50 pts). Thresholds double
...
/**
 * `airedSend` deliberately equals DUST_POINTS.send: one aired post is worth about an hour of idling
 * (or ~25 messages), which is the only ordering that matches what the product is for. At 10 it was
 * worth ten idle minutes, so the level was mostly a measure of leaving a tab open.
 */
export const LEVEL_POINTS = { message: 2, watchMinute: 1, airedSend: 50 } as const;
